...
class PointReader(BaseReader):
    ...
    def to_xarray(self, df: Union[pd.DataFrame, "dd.DataFrame"]) -> xr.Dataset:
        ...
        # 3. Standard Path (Consistently use 1D for both Eager and Lazy)
        # To ensure bit-perfect matching and maintain laziness for Dask,
        # we return a 1D dataset (long format) by default.
        if is_dask:
            ds = xr.Dataset()
            ...
            for col in temp_df.columns:
                series = temp_df[col]
                # Force string columns to object to avoid nullable string issues
                if pd.api.types.is_string_dtype(series):
                    series = series.astype(object)
                ds[col] = (("node",), series.to_dask_array(lengths=True))
        else:
            # For Pandas, we also use the 1D path for consistency.
            # We reset index to ensure 'node' is a simple integer range.
            ds = temp_df.reset_index(drop=True).to_xarray()
            if "index" in ds.dims:
                ds = ds.rename({"index": "node"})

        # Set standard coordinates for both paths
