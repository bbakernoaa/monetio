...
def modisClient(
    ...
    if client is None: client = setClient()
    m.server = client.wsdl.url

    if product is None: return client.service.getproducts()
    m.product = product

    if band is None: return client.service.getbands(product)
    m.band = band

    if lat is None or lon is None: raise ValueError("Lat/Lon needed")
    m.latitude = lat
    m.longitude = lon

    dateList = client.service.getdates(lat, lon, product)
    if startDate is None or endDate is None: return dateList
    ...
    while i < dateList.__len__() - 1:
        ...
        while mkIntDate(dateList[i + j - 1]) > endDate: j = j - 1
