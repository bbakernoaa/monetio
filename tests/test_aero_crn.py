...
def mock_crn_file(tmp_path):
    ...
    content = (
        "63893 20230101 1200 20230101 0600 1.000 -87.88 30.54 10.5 10.0 11.0 9.5 0.0 " + "0.0 " * 22 + "\n"
        "63893 20230101 1300 20230101 0700 1.000 -87.88 30.54 10.6 10.1 11.1 9.6 0.0 " + "1.0 " * 22 + "\n"
    )
...
def test_crn_eager_lazy_consistency(mock_get_monitor, mock_crn_file):
    monitor_df = pd.DataFrame({
        "WBAN": ["63893"], "STATE": ["AL"], "LOCATION": ["Fairhope"], "VECTOR": ["Test"],
        "LATITUDE": [30.54], "LONGITUDE": [-87.88], "NETWORK": ["USCRN"], "GMT_OFFSET": [-6.0],
    })
...
def test_crn_build_urls_optimization():
    ...
    monitor_df = pd.DataFrame({"STATE": ["AL"], "LOCATION": ["Fairhope"], "VECTOR": ["Test"], "LATITUDE": [30.0], "LONGITUDE": [-80.0]})
