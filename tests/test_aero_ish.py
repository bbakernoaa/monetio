...
def mock_history():
    return pd.DataFrame({
        "station_id": ["72224400358", "99999912345"], "usaf": ["722244", "999999"], "wban": ["00358", "12345"],
        "latitude": [38.9, 40.0], "longitude": [-76.9, -80.0], "ctry": ["US", "US"], "state": ["MD", "PA"],
        "station name": ["Site 1", "Site 2"], "elev(m)": [20.0, 30.0],
...
def test_ish_eager_lazy_consistency(tmp_path, mock_history):
    ...
    fn.write_text(line1 + "\n")
    reader = ISHReader()
    def side_effect(self, dates=None): self.history = mock_history
...
def test_ish_lite_multi_site_resample(monkeypatch, tmp_path):
    def mock_read_history(self, dates=None):
        self.history = pd.DataFrame({
            "usaf": ["012345", "543210"], "wban": ["67890", "09876"], "latitude": [40.0, 41.0], "longitude": [-80.0, -81.0],
            "station_id": ["01234567890", "54321009876"], "ctry": ["US", "US"], "state": ["PA", "OH"],
            "station name": ["Test Station 1", "Test Station 2"], "elev(m)": [100.0, 200.0],
    ...
    with gzip.open(fn1, "wb") as f: f.write("2023 01 01 00  100  50 10132  270   50 0 0 0\n".encode())
    fn2 = tmp_path / "543210-09876-2023.gz"
    with gzip.open(fn2, "wb") as f: f.write("2023 01 01 00  200  80 10132  270   50 0 0 0\n".encode())
    reader = ISHLiteReader()
    ds = reader.open_dataset(files=[str(fn1), str(fn2)], as_xarray=True, resample=True, window="h", lazy=False)
