...
def test_read_ish_file_logic(tmp_path):
    ...
    assert df.ws.iloc[0] == 4.0
    assert df.ws.iloc[1] == 5.0
...
def test_ish_eager_lazy_consistency(tmp_path):
    ...
    with patch("monetio.readers.ish.ISH.read_ish_history"):
        with patch("monetio.readers.ish.ISH") as mock_ish_class:
            mock_ish_instance = mock_ish_class.return_value
            mock_ish_instance.history = mock_history
            mock_ish_instance.source = "ncdc"

            # Eager path
            ds_eager = reader.open_dataset(
                files=str(fn), as_xarray=True, lazy=False, resample=False
            )
