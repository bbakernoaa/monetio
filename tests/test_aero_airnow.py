...
def test_airnow_eager_vs_lazy_local(mock_airnow_file, monkeypatch):
    def mock_read_monitor(*args, **kwargs):
        return pd.DataFrame({
            "siteid": ["012345678", "999999999"], "latitude": [40.0, 40.0],
            "longitude": [-80.0, -80.0], "site_name": ["Site 1", "Site 2"],
        })
    import monetio.readers.airnow as airnow
    monkeypatch.setattr(airnow, "read_monitor_file", mock_read_monitor)
    reader = AirNowReader()
    ds_eager = reader.open_dataset(files=mock_airnow_file, as_xarray=True, lazy=False, wide_fmt=True, bad_utcoffset="fix")
    ds_lazy = reader.open_dataset(files=mock_airnow_file, as_xarray=True, lazy=True, wide_fmt=True, bad_utcoffset="fix")
...
@pytest.mark.parametrize("date", [pd.Timestamp("2021/07/01"), pd.Timestamp("2024/04/23")], ids=["multiple_bad", "some_bad"])
def test_check_zero_utc_offsets_network(date, bad_utcoffset, request):
    dates = [date]
    df = AirNowReader().open_dataset(dates=dates, daily=False, wide_fmt=True, bad_utcoffset=bad_utcoffset, as_xarray=False)
    ...
    if bad_utcoffset == "leave": assert not bad_sites.empty
    elif bad_utcoffset in ["null", "drop", "fix"]: assert bad_sites.empty
    if bad_utcoffset == "fix": assert ((df.utcoffset >= -12) & (df.utcoffset <= 14)).all()
